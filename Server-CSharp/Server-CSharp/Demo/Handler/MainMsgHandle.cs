...
namespace YT
{
    public partial class MsgHandler
    {
        ...
        public static void MsgGetPlayerIntroduction(Connection c, byte[] bytes) {
            ...
            Player player = Player.Players[c.Conv];
        ...
        public static void MsgSavePlayerIntroduction(Connection c, byte[] bytes) {
            ...
            //保存数据
            DBManager.UpdatePlayerData(Player.Players[c.Conv].id, Player.Players[c.Conv].data);
            player.Send(msg);
        }
        ...
        public static void MsgGetFriendList(Connection c, byte[] bytes) {
            ...
            if (player == null) return;
            //获取FriendList
            msg.FriendList = DBManager.GetFriendList(Player.Players[c.Conv].id);
            msg.Result = 0;
            player.Send(msg);
        }
        //添加好友
